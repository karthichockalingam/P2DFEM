...
namespace constants {
    ...
    inline const real_t UN(real_t ce) { return Un(ce) / phi_scale; }
    inline const real_t UP(real_t ce) { return Up(ce) / phi_scale; }
    inline const real_t DE(real_t ce) { return De(ce * ce_scale) / De_scale; }
    inline const real_t Kappa(real_t x) { return kappa(x * ce_scale) / kappa_scale; }
