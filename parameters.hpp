#pragma once
#include "mfem.hpp"

using namespace mfem;

namespace LGM50 {

    // Positive electrode.
    extern const real_t cp0 = 17038.; // Initial concentration, positive electrode, mol/m^3
    extern const real_t cpmax = 63104; // Maximum concentration, positive electrode, mol/m^3

    extern const real_t rp = 5.22e-6; // Positive particle radius (m).
    extern const real_t positive_electrode_thickness = 75.6e-6;

    extern const real_t eps_p = 0.335; // Electrode volume fraction.
    extern const real_t eps_p_fi = 0.0;
    extern const real_t eps_p_s = 1 - eps_p - eps_p_fi;

    extern const real_t kp_dim = 3.42e-6;
    extern const real_t Dp = 4.0e-15; // Diffusion coefficient, positive electrode
    extern const real_t Ap = 3 * eps_p_s / rp; // Positive electrode area (m^2).

    extern const real_t sig_p = 0.18; // Positive electrode conductivity (S/m).

    // Negative electrode.
    extern const real_t cn0 = 29866.; // Initial concentration, negative electrode, mol/m^3
    extern const real_t cnmax = 33133; // Maximum concentration, negative electrode, mol/m

    extern const real_t rn = 5.86e-6; // Negative particle radius (m).
    extern const real_t negative_electrode_thickness = 85.2e-6;

    extern const real_t eps_n = 0.25; // Electrode volume fraction.
    extern const real_t eps_n_fi = 0.0;
    extern const real_t eps_n_s = 1 - eps_n - eps_n_fi;

    extern const real_t kn_dim = 6.48e-7;
    extern const real_t Dn = 3.3e-14; // Diffusion coefficient, negative electrode
    extern const real_t An = 3 * eps_n_s / rn; // Negative electrode area (m^2).

    extern const real_t sig_n = 215.; // Negative electrode conductivity (S/m).

    // Separator.
    extern const real_t separator_thickness = 12e-6;
    extern const real_t ce0 = 1000.0;
    extern const real_t tplus = 0.2594;
    extern const real_t eps_s = 0.47;
    extern real_t De(real_t ce)
    {
        return 8.794e-11 * pow(ce / 1000, 2) - 3.972e-10 * (ce / 1000) + 4.862e-10;
    }
    extern real_t kappa(real_t x)
    {
        return 0.1297 * pow(x / 1000, 3) - 2.51 * pow(x / 1000, 1.5) + 3.329 * (x / 1000);
    }

    // Cell parameters.
    extern const real_t cell_length = 1.58;
    extern const real_t cell_width = 6.5e-2;
    extern const real_t cell_no_layers = 1;
    extern const real_t cell_area = cell_width * cell_length * cell_no_layers;

    extern const real_t I_typ = 5.0;    // Or I1C in Jubat.

    extern const real_t brugg = 1.5; // Currently the same in each region, may need to vary later.


}
