...
$$
\bar{j}=2 \bar{j}_{\mathrm{ex}} \sinh \left[0.5\left(\bar{\phi}_i-\bar{\phi}_{\mathrm{e}}-\bar{U}_i\right) / \bar{T}\right], \quad \bar{j}_{\mathrm{ex}}=\bar{m}_i c_{i, \mathrm{surf}}^{0.5} \bar{c}_{\mathrm{e}}^{0.5}\left(\bar{c}_{i, \mathrm{surf}}-\bar{c}_{i, \max }\right)^{0.5}
$$

**Given**: 
$`\left.\bar{\phi}_{\mathrm{n}}\right|_{\bar{x}=0}=\bar{\phi}_{\mathrm{n}0}=0`$

**Find**: 
the unknown reference potentials of the positive electrode $`\bar{\phi}_{\mathrm{p}0}`$ and the electrolyte $`\bar{\phi}_{\mathrm{e}0}`$.

**Solution**:
1. Introduce the prime variables to separate:
   - the spatially varying part of the potentials and
   - the constant reference offsets $`\bar{\phi}_{n0}`$, $`\bar{\phi}_{p0}`$, $`\bar{\phi}_{e0}`$.
...
2. Substituting Butler-Volmer equation 

$$
\bar{j}=2 \bar{j}_{\mathrm{ex}} \sinh \left(\bar{\eta}_i / (2 \bar{T})\right)
$$
...
1. *Loop for solving the charge-conservation PDEs*:
   ...
   1.3. Compute the reference potentials $`\bar{\phi}_{e0}^{(m)}`$, $`\bar{\phi}_{p0}^{(m)}`$, with $\bar{\phi}_{n0}=0$.
